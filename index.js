...
function respondAnswer() {
    const answerText = document.getElementById('respondAnswer').value;
    conn.respondToAnswer(answerText);
}
...
async function connect() {
    ...
    const answerText = document.getElementById('connect').value;
    await conn.connectToOffer(answerText);
}


conn.onMessage((txt) => {
    document.getElementById('lastMsg').innerText = txt;
    if(isClient) {
        const videoElement = document.getElementById('vid');
        ...
        switch(f.type) {
            ...
            case 'pause':
                ...
                break;
            case 'seeked':
                videoElement.currentTime = f.value;
                break;
            default:
                break;
        }
...
conn.onLastIce((localOffer) => {
    document.getElementById('offer').value = localOffer;
});
...
setTimeout(() => {
    ...
    videoElement.onseeked = handleSeek;
    videoElement.onfullscreenchange = handleFullScreen
    videoElement.addEventListener('seeked', () => {
        console.log('actual');
    })
    function handleFullScreen(ev) {
        conn.sendMessage(JSON.stringify({
            type: ev.type,
            value: ev.target
        }))
    }
